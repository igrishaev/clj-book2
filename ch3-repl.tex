...
Подготовим словарь запроса. Мы передаем параметр \code{type=twopart}, чтобы шутка состояла из двух частей (подробней об этом ниже):
...
Еще одно замечание касается строки \code{(if e ...)} \coderef{11}. Мы проверяем на истину именно исключение, а не результат, потому что результат вполне может быть \code{nil}.
