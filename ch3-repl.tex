...
Предположим, мы бы хотели, чтобы команда "classpath" вернула текущий список \footurl{путей JVM}{\urlclasspath}[Class path]. Для этого напишем middleware с логикой: если поле \code{:op} сообщения равно "classpath", отправить сообщение со списком строк. В противном случае вызвать обработчик по умолчанию:
