...
Проект Cider, который мы скоро рассмотрим, предлагает \footurl{множество подобных middleware}{\urlcidermid}[Cider middleware]. Вместе они радикально расширяют возможности nREPL.
...
Пользователи Emacs догадались, что буфер \code{*cider-scratch*}~--- это аналог обычного \code{*scratch*}. Так называется \footurl{встроенный буфер}{\urlemacsscratch}[Emacs scratch] Emacs, который выполняет код на ELisp. Разница в том, что \code{*cider-scratch*} ожидает код на Clojure и выполняет его в том проекте, в папке с которым находится.
...
\item
  Запустить образ и подключиться к локальному порту из Emacs.
