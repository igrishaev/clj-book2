...
Наведите курсор на символ \code{client/request} и выполните \code{M-.} (команда \code{M-x cider-find-var}). Откроется модуль \code{clj-http.client} из jar-файла в директории \code{~/.m2/repository/clj-http/clj-http/3.12.0}. Вы окажетесь на строке 1134, где объявлена переменная \code{request}:
