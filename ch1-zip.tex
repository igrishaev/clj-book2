...
Термин <<зиппер>> ввел ученый \footurl{Жерар Юэ}{\huet}[G\'{e}rard Huet][-10mm]
...
\textbf{двойку}, обернем данные в зиппер и выполним команды \code{zip/down} и
...
Функция \code{zip/vector-zip} создает зиппер из вектора. Вызовы \code{zip/down} и
\code{zip/right} передвинут указатель на двойку, как и ожидалось. Последний шаг
\code{zip/node} вернет значение (узел) текущей локации. Если убрать \code{zip/node},
...
Как и в случае выше, от исключения вас убережет макрос \code{some->}.
...
\code{children}. Сам по себе словарь~--- это ветка, чью потомки~--- элементы
...
Поясним основные моменты. Композиция \code{(some-fn ...)} \coderef{3} вернёт
истину, если хотя бы один из предикатов сработает положительно. Иными словами,
на роль ветки мы рассматриваем только словарь или его узел (пару ключ-значение).
...
его значение вложенным словарём (функция \code{val} вернет второй элемент
...
странице справа. На вершине XML находится узел \code{catalog}. Это
группировочный тег: он необходим, потому что на вершине не может быть несколько
тегов. Потомки каталога~--- организации. В атрибуте \code{name} организации
указано её имя. Под организацией идут товары~--- узлы с тегом \code{product} и
атрибутом \code{type}. Вместо потомков товар содержит текст~--- его
наименование. Ниже товара спуститься уже нельзя.
...
\caption{Поставщики и товары в XML}
\label{fig:xml-suppliers}
...
Вызов \code{(->xml-zipper "products.xml")} вернет первую локацию зиппера
...
на элементы по всем четырем направлениям. Наоборот, \code{tree-seq} итерирует
...
состояние и хорошо смотрелась бы в императивных языках, но не в Clojure.
...
данные в JSON, их структура и обход отличаются кардинально. Сравните товары без
филиалов:
...
Очевидно, для работы с этими файлами нужен разный код. В случае с XML его
...
всего с устройством \coderef{10}. Нас просят найти магазин, где тряпочку можно купить
отдельно \coderef{4}.
...
комплекте с тряпочкой. Оба этих товара продаются в re-Store по отдельности, что
не подходит. Если заменить в наборе тряпочку на гарнитуру, мы не получим ни один
магазин.
...
Видим, что новый вектор (форма \code{(vec children)}, строка 6) копирует метаданные прежнего
вектора (переменная \code{node}). Если дополнить оригинал через \code{assoc} или
...
локацией, а потомок ниже. В центре рисунка находится локация с вектором \code{[2
...
Другой пример~--- поменять во вложеном векторе все \code{nil} на 0, чтобы
...
Видим, что цена на айфоны изменилась на 10\% (строки 7 и 17), а у остальных
товаров осталась прежней.
...
Итоговый XML, где в наборах появился новый товар \coderef{20}, приведен в
листинге \ref{fig:chart-xml-03} справа.
...
валюте \code{to}. Пусть это будут йены. Это значит, мы соединили \code{from} и
...
\code{zip/path}. Она вернёт родителей локации, не включая ее саму. Путь к
...
Цифра \code{3}, идущая перед \code{4} в строках 11 и 12, говорит о том, что
...
Полный код функции \code{exchange2} приведен на следующем развороте в листинге
...
\code{(take N)} сразу после \code{loc-layers}:
...
Библиотека \footurl{Hickory}{\hickory}[Hickory][3mm] предлагает парсер разметки
...
\code{make-node}, потому что они приватные. После вызова \code{with-meta} \coderef{4} локация из файла
...
подходят зипперу, нужно только описать функции \code{branch?}~--- может ли
элемент иметь потомков~--- и \code{children}~--- как именно их найти.
